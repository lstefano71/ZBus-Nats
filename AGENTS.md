...
### High-precision timing in APL

Use `16 ⎕DT 'Z'` for nanosecond-precision timestamps (no ⎕TS encoding needed).
The ns value since 2000-01-01 is ~8.3×10¹⁷ in 2026, exceeding IEEE double's ~15.9
significant digits. Set `⎕FR←1287` (128-bit DECF) **inside a dfn** so it's localised
and doesn't pollute the rest of the script:

```apl
elapsed_s←{⎕FR←1287 ⋄ (⍵-⍺)÷1e9}   ⍝ dfn localises ⎕FR

t0←16 ⎕DT 'Z'
⍝ ... work ...
t1←16 ⎕DT 'Z'
elapsed←t0 elapsed_s t1              ⍝ seconds with ns precision
```
This is how Conga-Sharp benchmarks do it. Much cleaner than `24 60 60 1000⊥¯4↑⎕TS`.
